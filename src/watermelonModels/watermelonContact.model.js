...
// TODO: @relation isn't working. Not sure why
...
class WatermelonContact extends Model {
  ...
  getActionForChild(tableName) {
    ...
    return (newChildren) => {
      return newChildren.map((newChild) => {
        return this.collections.get(tableName).prepareCreate((child) => {
          child.contact.set(this);
  ...
  getActionForRelation(tableName, contactKey) {
    // Assumes no inter-dependencies
    return (newRelation) => {
      return this.collections.get(tableName).prepareCreate((aRelation) => {
        this.update((contact) => {
          contact[contactKey].set(aRelation);
        });
...
class WatermelonAddress extends Model {
  ...
  @relation(CONTACT_TABLE_NAME, 'contact_id') contact;
}
